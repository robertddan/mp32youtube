...
define("PATH", substr(getcwd(), 0, -6)); //return directory where index.php is, with "/" (if this file is in folder "inc")
...
if (!empty($_FILES)) {
    ...
    if($type == 'image'){
        ...
        if ($_FILES['Filedata']['size'] >= MAX_SIZE_IMG) { 
            echo json_encode(array("error", "File size", "This file is too big. Size limit: " . $iusl[0]." ".$iusl[1].".", $name)); 
            return; 
        }

        if(empty($file_info)) {
            echo json_encode(array("error", "Corrupt File", "We could not read this file because it is corrupt." , $name));
            return true;
        }
        ...
        if(move_uploaded_file($tempFile,$targetFile)){
           if(file_exists(THB)){
                ...
                echo json_encode(array('id' => $rand_str, 
                            'name' => $file_name, 
                            'nameimg' => $name, 
                            'ext' => $ext,
                            'cate' => count($_SESSION['img'])));
    ...
    if($type == 'mp3'){
        ...
        //	echo json_encode(array("error", "<p>Song is too short</p>", $name . "' is too short (" . $fileinfo['playtime_seconds'] . "s, must be more than 10 seconds long). ", $name)); 
        //	return;  
        //}
